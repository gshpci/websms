...
if( substr($Number, 0, 4) == "0817" ||
    ...
    substr($Number, 0, 4) == "+63944" ||){
        ...
        $sql       = "INSERT INTO logstrigger(Station, `action`, datetime) VALUES ('{$station}', concat('Edit Contact',' - ','{$lName}',', ','{$fName}',' ','{$mName}') , concat(curdate(),' ',curtime()))";
            ...
            mysql_query("update phonebook set Number='$Number', lastname='$lName', firstname='$fName', middlename='$mName' where id='$id'");

            echo "<script>alert('Record successfuly updated.');window.location.href='sms.php';</script>";
